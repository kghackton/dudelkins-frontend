...
export const Map_Odh_Events = {
    state: {
        events: {},//мапа по id сорсов из мап по id лейблов

        loadingSources:[], //id сорсов, по которым производится загрузка
        images: {},//мапа по id image
    ...
    getters: {
        ...
        MAP_IMAGES:state=>state.images,
        MAP_LABELS_COLOR_LIST: (state, getters)=> getters.DICT_LABELS
            .map(labelInfo=>[labelInfo.number, labelInfo.color || 'rgba(0,0,0,0)'])
            .sort((a,b)=> a.number - b.number),
        MAP_EVENTS_FILTERED: (state, getters)=>{
            return Object.entries(state.events)
                .filter(([sourceUuid, events])=>getters.MAP_FILTER_ACTIVESOURCES.includes(sourceUuid))
                .flatMap(([sourceUuid, events])=>
                    Object.entries(events)
                        .filter(([label, events])=>getters.MAP_FILTER_LABELS.includes(label))
                        .flatMap(([label, events])=>events)
                )
        },
        MAP_EVENTS_FILTERED_FEATURED:(state, getters)=>{
            const events = getters.MAP_EVENTS_FILTERED
            console.log(events)
            const imageAcc = events.reduce((acc, ev )=>{
                const imageUuid= ev.imageUuid
                if(acc[imageUuid]){
                    acc[imageUuid].push(ev)
                } else {
                    acc[imageUuid] = [ev]
                }
                return acc
            }, {})
            const res = Object.entries(imageAcc).map(([imageUuid, ev])=>{
                const img = getters.MAP_IMAGES[imageUuid]
                if (!img){return null}
                const labelNums = ev.map(event=>{
                    return getters.DICT_LABELS.find(lbl=>lbl.uuid===event.labelUuid)?.number || 100
                })
                return {
                    "type": "Feature",
                    "properties": {
    ...
    actions:{
        MAP_SET_POPUP:({getters})=>{
            ...
            // getters.MAP.on('click', 'unclustered-point', handler)
        },
...
//         },
        ...
        MAP_EVENTS_INIT: ({dispatch, getters})=>{
        ...
        MAP_EVENTS_SOURCE_ADD:({commit,getters,dispatch}, newsource)=>{
            ...
            const now = new Date()
            const start = getters.MAP_FILTER_STARTTIME ? new Date(getters.MAP_FILTER_STARTTIME) : null
            const end = getters.MAP_FILTER_ENDTIME ? new Date(getters.MAP_FILTER_ENDTIME) : null

            const shouldUpdateOnline = !end || (now < end)

            if(shouldUpdateOnline){
                dispatch('WS_FILTERS_ADD_VALUES', {
                    channel: "image",
                    filters:{
                        sourceUuid: [newsource]
                    }
                })
            }
